...
\title{Label Entropy as a Predictor of LLM Necessity: An Empirical Decision Framework for Classifier Selection Across Four Domains}
...
\begin{highlights}
\item Label entropy $H(Y)$ predicts when LLMs outperform traditional ML classifiers.
\item Traditional ML matches LLMs on tasks with $H < 2$ bits (4 effective classes).
\item SVM collapses from 90.9\% to 24.3\% as entropy increases from 1.24 to 3.75 bits.
\item Entropy Decision Function reduces classifier selection from weeks to 30 seconds.
\item Framework validated across cybersecurity, news, emotion, and legal domains.
\end{highlights}
...
\begin{keyword}
task complexity \sep Shannon entropy \sep classifier selection \sep decision framework \sep LLM evaluation
\end{keyword}
...
\begin{table}[!ht]
\centering
\caption{Entropy-based classifier selection framework: threshold values and decision rules for choosing between traditional ML and LLMs}
\label{tab:framework}
\begin{tabular}{ccll}
\toprule
...
\begin{table*}[t]
\centering
\caption{Performance by Method and Entropy Level (5 seeds per LLM experiment)}
\label{tab:results}
\begin{tabular}{llccccccr}
\toprule
\textbf{Domain} & $\mathbf{H(Y)}$ & $K$ & \textbf{DT} & \textbf{SVM} & \textbf{BERT} & \textbf{LLM-0.5B} & \textbf{LLM-9B} & \textbf{Best ML$\to$LLM Gap} \\
\midrule
SALAD & 1.24 & 8 & .874 & \textbf{.909} & .814 & .875 & 1.000 & +9.1\% \\
AG News & 2.00 & 4 & .581 & .882 & \textbf{.923} & $.909 \pm .010$ & --- & $-$1.4\% (BERT wins) \\
GoEmotions & 3.75 & 28 & .173 & .243 & .335 & $.421 \pm .014$ & --- & \textbf{+8.6\%} \\
LedGAR & 6.16 & 100 & .183 & \textbf{.654} & .615 & $.622 \pm .028$ & --- & $-$3.2\% (SVM wins) \\
\bottomrule
\multicolumn{9}{l}{\scriptsize LLM-0.5B = Qwen2.5-0.5B QLoRA, 5 seeds each. Strict macro-F1.}
\end{tabular}
\end{table*}
...
At $H = 2.00$, SVM (88.2\%) approaches BERT (92.3\%), a gap of only 4.1 points. DT drops to 58.1\%---the feature-based approach begins to struggle with 4 balanced classes. Notably, the 0.5B LLM achieves $0.909 \pm 0.010$ across 5 seeds, \emph{slightly below} BERT (92.3\%). This confirms the transition zone: both BERT and fine-tuned LLMs provide comparable performance, while SVM remains competitive.

\textbf{P1 validation}: Gap = $-$1.4\% (LLM slightly below BERT, confirming the transition zone where method choice has minimal impact).

\subsubsection{GoEmotions ($H=3.75$ bits): LLM Essential}
At $H = 3.75$, SVM collapses to 24.3\%---barely above random for 28 classes. DT is even worse at 17.3\%. BERT reaches 33.5\%, still far below production requirements. The 0.5B LLM achieves $0.421 \pm 0.014$ across 5 seeds---\textbf{+8.6\% above BERT}. This confirms LLM necessity: only language models can leverage semantic understanding of fine-grained emotion labels.

\textbf{P1 validation}: ML-LLM gap = +8.6\% (LLM vs BERT), confirming LLM advantage above $H > 3$.
...
At $H = 6.16$, SVM achieves a surprising 65.4\%---higher than both BERT (61.5\%) and the 0.5B LLM ($0.622 \pm 0.028$). This confirms the ``LedGAR exception'': domain-specific vocabulary patterns (``pursuant to Section...'') provide more discriminative power than contextual embeddings from a 0.5B-parameter model. With 100 classes and only 5K training samples, the LLM lacks sufficient capacity for this regime.

\textbf{P1 validation}: ML-LLM gap = $-$3.2\% (SVM \emph{beats} LLM---the exception).
\textbf{P2 validation}: 5K samples is insufficient for 100 classes (estimated need: $>$20K).
...
P1: Gap($H$=2.00) & Moderate & $-$1.4\% (BERT$>$LLM) & \checkmark$^*$ \\
P1: Gap($H$=3.75) & Large ($>$30\%) & +8.6\% (LLM$>$BERT) & \checkmark \\
P1: Gap($H$=6.16) & Large ($>$30\%) & $-$3.2\% (SVM$>$LLM) & $\times$$^\dagger$ \\
P2: $N$(SALAD) & Few-shot & 1K semantic~\cite{p6} & \checkmark \\
P3: Halluc(SALAD) & $<$5 types & 1--4 types~\cite{p3} & \checkmark \\
\bottomrule
\multicolumn{4}{l}{\scriptsize $^*$Transition zone: BERT/LLM/SVM all within 4\%. $^\dagger$LedGAR: SVM wins due to distinctive vocabulary.}
\end{tabular}
\end{table}
...
\textbf{LLM model size}: Cross-domain LLM experiments used a 0.5B model, which may underperform larger models (7B+) especially on LedGAR's 100-class task. The LedGAR exception may vanish with larger models.

\textbf{Training size}: All LLM experiments used 5K samples. LedGAR's 100 classes likely require $>$20K samples for the LLM to outperform SVM.
...
Entropy computed from label distributions using $H(Y) = -\sum p_k \log_2 p_k$. Traditional ML via scikit-learn. BERT via HuggingFace Transformers. LLM via LlamaFactory v0.9.2 on A100 80GB. Code at \url{https://github.com/nutthakorn7/soc-llm-research}.
...
\bibitem{moustafa2016} N.~Moustafa and J.~Slay, ``UNSW-NB15: A Detailed Data Set for Network Intrusion Detection Systems,'' in \emph{Proc.\ MilCIS}, 2015.
