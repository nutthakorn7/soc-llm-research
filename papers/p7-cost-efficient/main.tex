...
\title{Sub-Dollar Fine-Tuning: Cost--Compliance Trade-offs in Parameter-Efficient LLM Adaptation for Security Operations}
...
We provide the first full economic analysis of parameter-efficient fine-tuning for SOC alert classification, benchmarking 6 models across all four cost dimensions. Our key findings:
\begin{enumerate}
...
\begin{table}[!ht]
\centering
\caption{Training profiles for 7 LLMs: GPU time, VRAM usage, cost per run, and cost-per-F1-point efficiency}
\label{tab:profiles}
\begin{tabular}{lrrcc}
\toprule
...
The cost-performance landscape reveals a clear Pareto frontier: SVM (\$0, 90.9\%) $\to$ Phi4-mini (\$0.60, 100\%) $\to$ DeepSeek-R1 (\$0.70, 100\%). All other models are Pareto-dominated: they cost more \emph{and} perform worse than at least one frontier model. Three patterns stand out:
\begin{itemize}
...
Fine-tuning is \textbf{2,055$\times$} more cost-efficient than commercial APIs per F1 point gained over SVM. Fine-tuned models achieve 100\% strict F1 while ICL achieves only $\sim$95\%, making the comparison even more favorable.
...
All experiments use LlamaFactory v0.9.2 on NVIDIA A100 80GB. Pricing based on \$2/GPU-hr (AWS/GCP average). Code at \url{https://github.com/nutthakorn7/soc-llm-research}.
