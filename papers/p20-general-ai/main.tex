...
\item \textbf{Multi-seed robustness}: 0.5B achieves $92.0 \pm 0.9$\% F1 across 4 seeds, demonstrating low variance.
...
\midrule
Qwen2.5-0.5B & 4 & $0.920{\pm}0.009$ & $0.920{\pm}0.009$ & 95m$^\dagger$ & \$0.37 \\
Qwen2.5-7B & 1 & 0.911 & 0.911 & 92.3m & \$3.08 \\
\bottomrule
\multicolumn{6}{l}{\scriptsize $^\dagger$Mean of 69--121m across T4 GPUs (Colab, Kaggle).}
\end{tabular}
\end{table}
...
\midrule
77 & 0.9154 & 0.916 & 1 & 121m \\
123 & 0.9300 & 0.930 & 0 & 121m \\
456 & 0.9248 & 0.925 & 0 & 69m \\
999 & 0.9093 & 0.910 & 0 & 69m \\
\midrule
\textbf{Mean} & \textbf{0.9199} & \textbf{0.9203} & \textbf{0.25} & --- \\
\textbf{Std} & $\pm$0.0090 & $\pm$0.0087 & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
With 4 seeds, the 0.5B model achieves $92.0 \pm 0.9$\% F1---\emph{surpassing} both the 7B single-seed result (91.1\%) and the SVM baseline (88.2\%). The low standard deviation ($\pm 0.9$\%) confirms robustness across random initializations. Only 1 hallucinated label was observed across all 4,000 test predictions (seed~77: 1 out-of-schema label), indicating near-perfect label compliance on AG News.
...
\midrule
AG News & AG News & $0.920{\pm}.009$ & $0.920{\pm}.009$ & In-domain \\
...
\caption{Cross-domain collapse: AG News-trained model achieves 92.0\% F1 in-domain (4 seeds) but absolute 0\% on GoEmotions. GoEmotions-trained model (45.3\% in-domain) also achieves 0\% on AG News. Domain lock-in is symmetric.}
...
\end{equation}
For Qwen2.5-0.5B: $F1(\text{AG}, \text{AG}) = 0.920 \pm 0.009$ (4 seeds) and $F1(\text{AG}, \text{GoEmo}) = 0.000$ --- complete domain lock-in.
...
\item \textbf{Robust in-domain}: 0.5B achieves $92.0 \pm 0.9$\% F1 across 4 seeds with near-zero hallucination (1/4,000).
...
\item Budget \textbf{0.5B for cost-sensitive} deployments ($92.0\%$ at \$0.37)---the 0.5B model now \emph{matches} the 7B (91.1\%) at $8.4\times$ lower cost.
