...
\begin{enumerate}
\item \textbf{In-domain}: Train and test on AG News at two model scales (0.5B, 7B).
...
\begin{table}[h]
\centering
\caption{In-Domain AG News Results}
\label{tab:indomain}
\begin{tabular}{lcccc}
\toprule
...
\begin{table}[h]
\centering
\caption{Cross-Domain: AG News $\to$ GoEmotions (0.5B)}
\label{tab:cross}
\begin{tabular}{lccc}
\toprule
\textbf{Condition} & \textbf{F1} & \textbf{Acc} & \textbf{Valid Labels} \\
\midrule
In-domain (AG$\to$AG) & 0.887 & 0.886 & 4/4 \\
Cross-domain (AG$\to$GoEmo) & \textbf{0.000} & \textbf{0.000} & 0/28 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Cross-domain collapse: AG News-trained model achieves 88.7\% F1 in-domain but absolute 0\% on GoEmotions. Fine-tuning creates domain-locked classifiers.}
...
\begin{table}[h]
\centering
\caption{Vocabulary Leakage Classification}
\label{tab:leakage}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[h]
\centering
\caption{Traditional ML Performance by Task Entropy}
\label{tab:entropy}
\begin{tabular}{lcccr}
\toprule
...
\begin{table}[h]
\centering
\caption{Domain Routing Options}
\label{tab:routing}
\begin{tabular}{lccc}
\toprule
...
P14~\cite{p14} & OFT s123 & 71\% F1 & Switch to LoRA \\
\bottomrule
\end{tabular}
\end{table}

Fine-tuned LLMs fail at every type of generalization test: novel classes (P18), novel domains (P20), and even post-alignment (P9). The common thread: 0.5B models with QLoRA have insufficient capacity to maintain pre-training knowledge alongside fine-tuned behavior. Only P14's OFT partially preserves pre-training---at the cost of compliance instability.
