...
\begin{document}
\title{Model Scale and Label Compliance: How Sub-Billion Parameter LLMs Amplify Hallucination in Fine-Tuned Classification}
...
\begin{table}[!ht]
\centering
\caption{Seven-model evaluation suite spanning 0.5B--9B parameters: architectures, sizes, and reasoning capabilities}
\label{tab:models}
\begin{tabular}{lrccc}
\toprule
...
\begin{table}[!ht]
\centering
\caption{Unified training configuration ensuring fair comparison: identical QLoRA settings, data splits, and evaluation pipeline}
\label{tab:config}
\begin{tabular}{ll}
\toprule
...
\begin{table}[!ht]
\centering
\caption{Error taxonomy by model: hallucination types (sub-technique, plural, cross-category) vary by architecture family}
\label{tab:errors}
\begin{tabular}{lrrl}
\toprule
...
\begin{table}[!ht]
\centering
\caption{Two architectural paths to perfect label compliance: instruction-tuned (Phi4-mini) and reasoning-capable (DeepSeek-R1)}
\label{tab:paths}
\begin{tabular}{lcccc}
\toprule
...
To verify that sub-1B model performance generalizes beyond SALAD, we evaluate Qwen2.5-0.5B on AG News (4-class news classification, $H = 2.0$ bits) with 4 seeds:

\begin{table}[!ht]
\centering
\caption{Cross-domain validation: Qwen2.5-0.5B on AG News (4 seeds)}
\label{tab:agnews}
\begin{tabular}{rccc}
\toprule
\textbf{Seed} & \textbf{F1} & \textbf{Acc} & \textbf{Hal} \\
\midrule
77 & 0.9067 & 0.908 & 0 \\
123 & 0.8945 & 0.896 & 0 \\
456 & 0.9207 & 0.921 & 0 \\
999 & 0.9044 & 0.905 & 0 \\
\midrule
\textbf{Mean} & $\mathbf{0.907 \pm 0.011}$ & $\mathbf{0.908 \pm 0.010}$ & \textbf{0.0} \\
\bottomrule
\end{tabular}
\end{table}

Three findings emerge: (1)~Qwen2.5-0.5B achieves $90.7 \pm 1.1$\% F1 on a 4-class task---substantially higher than its 55.7\% on 8-class SALAD, consistent with the entropy-complexity relationship. (2)~Zero hallucination (0 errors across 4,000 predictions) suggests that label compliance improves on lower-entropy tasks. (3)~Low variance ($\sigma = 0.011$) indicates that sub-1B compliance is reproducible when task complexity is within capacity.

\subsection{Threats to Validity}
\textbf{Internal}: Single seed (42) for the 7-model SALAD comparison; seed sensitivity may affect rank ordering. However, the binary nature of the reasoning effect (0 vs.\ thousands of errors) is unlikely to be seed-dependent. Cross-domain validation on AG News uses 4 seeds ($\sigma = 0.011$), confirming low variance for sub-1B models.

\textbf{External}: Primary evaluation on single task (SALAD, $H = 1.24$ bits). Cross-domain validation on AG News ($H = 2.0$ bits) shows consistent sub-1B performance but on a simpler task.
...
All experiments use publicly available models from HuggingFace with LlamaFactory v0.9.2, QLoRA rank 64, on NVIDIA A100 80GB GPUs. Clean data splits and evaluation code available at \url{https://github.com/nutthakorn7/soc-llm-research}. All results are reproducible with seed 42.
