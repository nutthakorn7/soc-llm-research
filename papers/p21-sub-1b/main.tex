...
999 & 0.9093 & 0.910 & 0 \\
\midrule
\textbf{Mean} & $\mathbf{0.920 \pm 0.009}$ & $\mathbf{0.920 \pm 0.009}$ & \textbf{0.25} \\
\bottomrule
\end{tabular}
\end{table}

Three findings emerge: (1)~Qwen2.5-0.5B achieves $92.0 \pm 0.9$\% F1 on a 4-class task---substantially higher than its 55.7\% on 8-class SALAD, consistent with the entropy-complexity relationship. (2)~Near-zero hallucination (1 error across 4,000 predictions) suggests that label compliance improves on lower-entropy tasks. (3)~Low variance ($\sigma = 0.009$) indicates that sub-1B compliance is reproducible when task complexity is within capacity.

\subsection{Threats to Validity}
\textbf{Internal}: Single seed (42) for the 7-model SALAD comparison; seed sensitivity may affect rank ordering. However, the binary nature of the reasoning effect (0 vs.\ thousands of errors) is unlikely to be seed-dependent. Cross-domain validation on AG News uses 4 seeds ($\sigma = 0.009$), confirming low variance for sub-1B models.
