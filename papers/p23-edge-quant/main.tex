...
\caption{Quantization has near-zero effect on label compliance. 4-bit ($89.4 \pm 0.7$\%) and 8-bit ($90.2 \pm 0.6$\%) achieve overlapping F1 ranges with zero hallucination across all 10 runs.}
\label{fig:compliance}
\end{figure}

\textbf{Key finding}: The difference between 4-bit and 8-bit quantization is \textbf{0.8 percentage points} ($p = 0.12$, Welch's $t$-test, n.s.). Both achieve zero hallucinated labels across all seeds. The variance is remarkably low ($\sigma \leq 0.007$), confirming that quantization introduces negligible stochastic effects on compliance.
