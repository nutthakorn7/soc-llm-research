...
\begin{document}
\title{Can Fine-Tuned LLMs Classify What They've Never Seen? Zero-Shot Generalization in News Classification}
\author{
...
\item A novel \textbf{confusion proximity analysis} revealing systematic semantic neighborhoods: held-out categories map to the closest training category.
...
Yin et~al.~\cite{yin2019} benchmarked zero-shot text classification using entailment-based approaches. Wei et~al.~\cite{wei2022} (FLAN) showed instruction-tuned models exhibit zero-shot abilities across unseen tasks. Pushp and Srivastava~\cite{pushp2017} used feature transfer from seen to unseen classes. The broader zero-shot learning literature~\cite{lampert2014,romera2015,wang2019,xian2019} establishes attribute-based and embedding-based approaches for unseen class recognition. Our setup differs fundamentally: we evaluate \emph{after} fine-tuning, testing whether task-specific training preserves or destroys general zero-shot capability.

\subsection{Catastrophic Forgetting}
McCloskey and Cohen~\cite{mccloskey1989} documented catastrophic interference. Kirkpatrick et~al.~\cite{kirkpatrick2017} proposed EWC for forgetting mitigation. Luo et~al.~\cite{luo2023} confirmed fine-tuning erases general LLM capabilities. Our work provides precise quantification: a single fine-tuning run of 3K samples (3 of 4 categories) completely eliminates the ability to generate the 4th label.

\subsection{Cybersecurity Context}
This experiment extends our SOC research program~\cite{p3,p6,p7,p21}. While SALAD has 8 categories (making 8 leave-one-out experiments possible), we use AG News (4 categories) for clarity and cost efficiency, with each experiment testing 500 samples. Ferrag et~al.~\cite{ferrag2024} surveyed LLMs for cybersecurity. The UNSW-NB15 dataset~\cite{moustafa2016} underlies SALAD. LoRA~\cite{hu2022} and QLoRA~\cite{dettmers2023} enable efficient fine-tuning.

\textbf{Novelty gap.} No prior work has (1)~quantified fine-tuning's destruction of zero-shot generalization via controlled leave-one-out experiments, (2)~analyzed predicted vocabulary distributions for held-out categories, (3)~demonstrated vocabulary collapse where common English words become unreachable after fine-tuning, or (4)~formalized confusion proximity matrices for fine-tuned classification models.
...
This creates 4 independent experiments. Model: Qwen2.5-0.5B-Instruct, QLoRA rank 64, seed 42, trained on Vast.ai A100.

\subsection{Categories}

\begin{table}[h]
\centering\caption{AG News Categories and Transfer Predictions}
...
\begin{table}[h]
\centering\caption{Leave-One-Out Results: All Categories Fail}
\label{tab:results}
\begin{tabular}{lcrrl}
\toprule
\textbf{Held-Out} & \textbf{Accuracy} & \textbf{Train Time} & \textbf{n=500} & \textbf{Dominant Prediction} \\
\midrule
Business & \textbf{0.0\%} & 11.1m & 500 & World (54.6\%) \\
Sci/Tech & \textbf{0.0\%} & 11.4m & 500 & Business (71.6\%) \\
Sports & \textbf{0.0\%} & 11.1m & 500 & World (94.4\%) \\
...
\textbf{Key finding}: Every experiment produces 0.0\% accuracy. The model \emph{cannot} generate the held-out category label under any circumstance. The model does not even produce the label once in 500 samples---the vocabulary item is completely unreachable after fine-tuning.

\subsection{Predicted Vocabulary Analysis}

\begin{table}[h]
\centering\caption{Complete Predicted Vocabulary Distributions}
\label{tab:vocab}
\begin{tabular}{lccc}
\toprule
\textbf{Held-Out} & \textbf{Pred 1 (\%)} & \textbf{Pred 2 (\%)} & \textbf{Pred 3 (\%)} \\
\midrule
Business & World (54.6) & Sci/Tech (44.8) & Sports (0.6) \\
Sci/Tech & Business (71.6) & World (27.4) & Sports (1.0) \\
Sports & World (94.4) & Sci/Tech (0.6) & Business (0) \\
World & Sci/Tech (56.8) & Business (37.4) & Sports (5.8) \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Confusion proximity matrix. Arrows show dominant confusion direction. Line width indicates confusion strength. Sports → World (94.4\%) is the strongest confusion.}
\label{fig:confusion}
\end{figure}

The distributions are non-uniform and semantically meaningful:
\begin{itemize}
\item \textbf{Sports $\to$ World (94.4\%)}: International sports coverage overlaps ``World'' news. Near-unanimous confusion.
\item \textbf{Sci/Tech $\to$ Business (71.6\%)}: Technology industry articles frequently discuss business aspects (earnings, IPOs, market impact).
\item \textbf{World $\to$ Sci/Tech (56.8\%)}: News about international tech policy, space programs, and scientific discoveries.
\item \textbf{Business $\to$ World (54.6\%)}: International business and trade news overlaps ``World.''
\end{itemize}
...
\midrule
Bus held out & --- & 44.8 & 0.6 & \textbf{54.6} \\
S/T held out & \textbf{71.6} & --- & 1.0 & 27.4 \\
Spt held out & 0.0 & 0.6 & --- & \textbf{94.4} \\
Wld held out & 37.4 & \textbf{56.8} & 5.8 & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Sports & World (94.4\%) & International sports \\
Sci/Tech & Business (71.6\%) & Tech industry \\
World & Sci/Tech (56.8\%) & Global tech policy \\
Business & World (54.6\%) & Int'l trade/economics \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
\item \textbf{Sports is closest to World}: Sports articles frequently mention countries, international events, and geopolitical context---making ``World'' the overwhelming semantic nearest neighbor (94.4\%).
\item \textbf{Sci/Tech is closest to Business}: Technology coverage in AG News focuses heavily on companies, earnings, IPOs, and market impact---reflecting the tech-industry overlap (71.6\%).
...
\begin{table}[h]
\centering\caption{Transfer Predictions vs.\ Actual Results}
\label{tab:transfer}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[h]
\centering\caption{Vocabulary Collapse Across Studies}
\label{tab:collapse}
\begin{tabular}{llcc}
\toprule
...
P14~\cite{p14} & OFT Seed 123 & Partial (10 halluc) & 0.5B \\
\bottomrule
\end{tabular}
\end{table}

Vocabulary collapse is consistent across 4 independent studies for Qwen2.5-0.5B: the model can only generate labels present in its fine-tuning data. The 10 hallucinated labels in P14's OFT experiment are the sole exception, and they occur precisely because OFT's orthogonal constraint partially preserves pre-training vocabulary.
...
\begin{table}[h]
\centering\caption{Generalization Failures Across Studies}
\label{tab:cross}
\begin{tabular}{llc}
\toprule
...
\textbf{Single seed}: Seed 42 only. Different initializations may produce slightly different confusion distributions.

\textbf{QLoRA only}: Full fine-tuning adapts all parameters and may produce even more severe vocabulary collapse. OFT (P14) produces less collapse due to orthogonal preservation.
...
\noindent\textbf{P19 Checklist Self-Audit}: This paper passes Items 1--4 (data integrity), 10--12 (metrics), 14 (seed=42 documented), 17 (vocab audited). Fails Item 14 (multi-seed) and Item 24 (cost per experiment not itemized). Overall: \textbf{22/30 pass}.
...
\item \textbf{Systematic confusion}: Held-out categories map to semantically nearest training categories (Sports $\to$ World: 94.4\%).
