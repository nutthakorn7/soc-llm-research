...
We test this with a controlled leave-one-out protocol on AG News, training on 3 of 4 categories and testing exclusively on the held-out category. We use AG News (4-class news, $H = 2.0$ bits) as a controlled proxy to isolate the target phenomenon from domain-specific confounds. Its balanced distribution and established benchmark status enable clean ablation. The results are unambiguous: \textbf{zero-shot generalization is completely destroyed by fine-tuning}, even for a model that demonstrably knew these labels before fine-tuning.
