...
\title{LoRA Rank, Data Quality, and Label Compliance: Disentangling the Capacity--Hallucination Trade-off in Parameter-Efficient Fine-Tuning}
...
Several LoRA variants address different aspects: AdaLoRA~\cite{zhang2023} adaptively allocates rank across layers based on importance scores. QLoRA~\cite{dettmers2023} adds 4-bit NF4 quantization for single-GPU fine-tuning of 7B+ models. DoRA~\cite{liu2024} decomposes weight updates into magnitude and direction components for improved stability. VeRA~\cite{kopiczko2024} reduces trainable parameters further by sharing random projections across layers. Biderman et~al.~\cite{biderman2024} systematically compared LoRA to full fine-tuning, finding that LoRA ``learns less and forgets less''---directly relevant to our rank-compliance analysis. Hayou et~al.~\cite{hayou2024} provided theoretical foundations for LoRA initialization and rank selection. A comprehensive survey of LLM capabilities~\cite{zhao2023} highlights that evaluation methodology remains a challenge---a theme central to our work. None of these studies examine rank's effect on \emph{label compliance} as distinct from accuracy.
...
\begin{table}[!ht]
\centering
\caption{Trainable parameters per LoRA rank configuration: from 8.4M (rank 16) to 67.1M (rank 128) on Qwen3.5-9B}
\label{tab:params}
\begin{tabular}{rrrr}
\toprule
...
\begin{table}[!ht]
\centering
\caption{Rank-32 performance analysis before and after data leakage correction: 100\% F1 was a leakage artifact}
\label{tab:r32}
\begin{tabular}{lccc}
\toprule
...
\subsection{Cross-Domain Verification (AG News)}

To verify that rank effects generalize beyond cybersecurity, we evaluate on AG~News (4-class) and cross-reference with SALAD (8-class) using Qwen2.5-0.5B-Instruct with 4-bit QLoRA across 5 random seeds per configuration.

\begin{table}[!ht]
\centering
\caption{Cross-domain rank verification: AG~News (5 seeds per rank)}
\label{tab:cross-domain}
\begin{tabular}{rccc}
\toprule
\textbf{Rank} & \textbf{F1 (mean $\pm$ std)} & \textbf{Halluc.} & \textbf{Params} \\
\midrule
16 & $0.908 \pm 0.022$ & 0 & 1.1M \\
32 & $0.906 \pm 0.018$ & 0 & 2.1M \\
64 & $0.909 \pm 0.013$ & 0 & 4.2M \\
128 & $0.910 \pm 0.020$ & 0 & 8.4M \\
\bottomrule
\end{tabular}
\end{table}

AG~News is \textbf{rank-saturated}: all ranks from 16 to 128 achieve equivalent F1 ($0.906$--$0.910$, $\Delta < 0.4\%$) with zero hallucination. This confirms that on low-entropy tasks ($H=2.0$ bits), the rank-compliance tradeoff observed on SALAD with Qwen3.5-9B does not manifest---the 0.5B model lacks sufficient pre-training knowledge to hallucinate MITRE taxonomy terms.

\begin{table}[!ht]
\centering
\caption{SALAD rank scaling: r=4 vs r=128 (5 seeds)}
\label{tab:salad-rank}
\begin{tabular}{rccc}
\toprule
\textbf{Rank} & \textbf{F1 (mean $\pm$ std)} & \textbf{$\Delta$ from r=4} \\
\midrule
4 & $0.583 \pm 0.164$ & --- \\
128 & $\mathbf{0.852 \pm 0.136}$ & +26.9\% \\
\bottomrule
\end{tabular}
\end{table}

Contrary to the Qwen3.5-9B result (where lower rank = better compliance), the 0.5B model on SALAD shows the \emph{opposite}: r=128 outperforms r=4 by 26.9\%. This suggests that the rank-compliance tradeoff (Definition~1) is \textbf{model-size-dependent}: smaller models need higher rank for capacity, while larger models need lower rank to suppress pre-training vocabulary.
...
All experiments use Qwen3.5-9B from HuggingFace with LlamaFactory v0.9.2 on NVIDIA A100 80GB. Clean data splits and evaluation code at \url{https://github.com/nutthakorn7/soc-llm-research}.
...
\bibitem{hayou2024} S.~Hayou et~al., ``LoRA+: Efficient Low Rank Adaptation of Large Models,'' in \emph{Proc.\ ICML}, 2024.
...
\bibitem{kopiczko2024} D.~J.~Kopiczko et~al., ``VeRA: Vector-Based Random Matrix Adaptation,'' in \emph{Proc.\ ICLR}, 2024.
