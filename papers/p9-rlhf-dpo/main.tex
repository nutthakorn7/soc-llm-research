...
\begin{table}[h]
\centering\caption{Four Paths to Label Compliance}
\label{tab:pipeline}
\begin{tabular}{lccrr}
\toprule
\textbf{Method} & \textbf{Stage 1} & \textbf{Stage 2} & \textbf{Cost} & \textbf{F1} \\
\midrule
SFT (5K) & SFT (5K) & --- & \$0.61 & 88.8\% \\
...
\caption{DPO catastrophic failure. SFT achieves 88.8\% strict F1 with zero hallucination. DPO destroys classification capability entirely: F1 = 0.0\%, hallucination rate = 100\%.}
\label{fig:collapse}
\end{figure}
...
\begin{table}[h]
\centering\caption{DPO Output Distribution (Sample)}
\label{tab:outputs}
\begin{tabular}{lr}
\toprule
...
\begin{table}[h]
\centering\caption{SFT Baseline Per-Class Performance}
\label{tab:sft}
\begin{tabular}{lccr}
\toprule
...
The SFT model achieves 88.8\% strict F1 with zero hallucinated label variants---every prediction is a valid schema label. The 814 errors are \emph{misclassifications} (correct vocabulary, wrong category), not vocabulary errors. DPO was tasked with improving accuracy from 88.8\% to 100\%, but instead destroyed it to 0\%.
...
\midrule
SFT (5K) & 88.8\% & \$0.61 & 18m & 5K & Partial \\
...
The clear winner is \textbf{reasoning architecture} (Phi4-mini): perfect compliance at \$0.60 with only 5K training samples. DPO is the worst option---worse than baseline SFT by 88.8 percentage points.
...
P14~\cite{p14} & LoRA vs.\ OFT & LoRA wins & \$0.63 & Additive override \\
...
\section{Conclusion}
We present the first application of DPO for label hallucination reduction in classification, discovering catastrophic failure: F1 drops from 88.8\% (SFT) to 0.0\% (SFT+DPO).
