...
\title{Mind the Label Gap: How Fine-Tuned LLMs Hallucinate Sub-Categories in SOC Alert Classification}
...
\begin{table}[h]
\centering
\caption{Evaluation Pipeline (Algorithm 1)}
\label{alg:eval}
\begin{tabular}{rl}
\toprule
...
\begin{table}[h]
\centering
\caption{Response Format Compliance Across Models}
\label{tab:format}
\begin{tabular}{lcccl}
\toprule
...
\begin{enumerate}
\item \textbf{High-entropy evaluation}: Extend the dual-metric framework to GoEmotions ($H=3.75$ bits, 28 classes) and LedGAR ($H=6.16$ bits, 100 classes) to test whether the label gap phenomenon scales with task complexity.
\item \textbf{Alignment techniques}: Apply DPO/ORPO to explicitly reward label compliance during training, potentially eliminating hallucination without requiring 20K samples.
\item \textbf{Constrained decoding}: Enforce valid label vocabulary at inference time using finite-state automata (Outlines, LMQL) to guarantee schema compliance regardless of model tendency.
\item \textbf{Post-hoc label layers}: Train a lightweight mapping head on top of frozen LLM representations to enforce label vocabulary compliance.
\item \textbf{Multilingual SOC}: Evaluate label compliance in non-English SOC environments (Thai, Japanese, Arabic), where pre-training vocabulary imbalance may amplify hallucination.
\item \textbf{Temporal generalization}: Test whether models trained on 2023 attack data maintain label compliance on 2025 attack patterns, evaluating concept drift effects on hallucination.
\end{enumerate}
