...
\title{A 30-Item Reproducibility Checklist for LLM-Based Classification: Standardizing Evaluation in Applied Domains}
...
We write from direct experience. Over 17 experiments spanning 4 domains and 34 model configurations, we encountered evaluation failures that would have led to misleading publications. Biderman et~al.~\cite{biderman2024} similarly documented that reproducible evaluation of language models requires careful attention to overlooked details, and Tikhonov and Yamshchikov~\cite{tikhonov2024} showed evaluation results in NLP are often non-reproducible due to metric and preprocessing choices. Our most striking example:
...
Ji et~al.~\cite{ji2023} provide a comprehensive taxonomy of hallucination types, distinguishing intrinsic (contradicting source) from extrinsic (adding unverifiable information) hallucination. Gekhman et~al.~\cite{gekhman2024} showed that fine-tuning on new knowledge paradoxically increases hallucination of existing knowledge---directly relevant because domain fine-tuning may activate pre-training vocabularies. Xu et~al.~\cite{xu2024} argued hallucination is an inherent LLM limitation. Huang et~al.~\cite{huang2023} surveyed hallucination detection and mitigation. Li et~al.~\cite{li2024halueval} introduced HaluEval, a benchmark for evaluating hallucination detection, while Min et~al.~\cite{min2023factscore} proposed FActScore for fine-grained factual precision---both addressing evaluation rigor but neither covering label vocabulary hallucination. Sun et~al.~\cite{sun2024} surveyed LLM safety through verification and validation lenses, reinforcing the need for structured evaluation protocols.
...
Ferrag et~al.~\cite{ferrag2024} surveyed LLMs for cybersecurity, reporting high F1 scores without strict evaluation. Motlagh et~al.~\cite{motlagh2024} benchmarked ChatGPT on 5 datasets (45--92\% F1) using standard metrics. Bhatt et~al.~\cite{secure2024} introduced the SECURE benchmark for cybersecurity LLM evaluation, though it focuses on knowledge tasks rather than classification compliance. LoRA~\cite{hu2022} and QLoRA~\cite{dettmers2023} enable efficient domain adaptation, with subsequent refinements including DoRA~\cite{liu2024dora} (weight decomposition) and AdaLoRA~\cite{zhang2023adalora} (adaptive rank allocation). Wang et~al.~\cite{wang2024} surveyed data selection for language models, highlighting how training data quality affects downstream evaluation---a theme central to our checklist. The UNSW-NB15 dataset~\cite{moustafa2016} underlies the SALAD dataset used in our case studies.
...
\begin{table}[!ht]
\centering
\caption{Checklist category overview: 9 categories, 30 items, covering data integrity through deployment readiness}
\label{tab:overview}
\begin{tabular}{lccl}
\toprule
...
\begin{table}[!ht]
\centering
\caption{Perfect Score Suspicion Protocol: 15-step procedure triggered when any metric reports $\geq$99\% to detect artifacts}
\label{tab:protocol}
\begin{tabular}{clcl}
\toprule
...
\begin{table}[!ht]
\centering
\caption{Case study summary showing error magnitude (up to 44.3\% inflation) and which checklist items detect each error}
\label{tab:cases}
\begin{tabular}{lccl}
\toprule
...
To validate beyond our own studies, we retrospectively applied our checklist to six published LLM evaluation papers spanning cybersecurity, medical, legal, and general NLP domains:

\begin{table}[!ht]
\centering
\caption{External audit of 6 published papers: LLM-specific checklist items (12, 17--20) are universally failed, confirming a systematic gap}
\label{tab:audit}
\begin{tabular}{lccp{3.5cm}}
\toprule
\textbf{Paper} & \textbf{Pass} & \textbf{Fail} & \textbf{Critical Gap} \\
\midrule
\multicolumn{4}{l}{\emph{Cybersecurity}} \\
Ferrag~\cite{ferrag2024} & 14/30 & 16/30 & No strict/norm (12), no vocab audit (17--18) \\
Motlagh~\cite{motlagh2024} & 11/30 & 19/30 & No leakage check (1), no multi-seed (14) \\
Bhatt~\cite{secure2024} & 18/30 & 12/30 & No strict/norm (12), no cost (24--26) \\
\midrule
\multicolumn{4}{l}{\emph{Medical NLP}} \\
Gekhman~\cite{gekhman2024} & 20/30 & 10/30 & No per-class F1 (11), no cost (24--26) \\
\midrule
\multicolumn{4}{l}{\emph{General NLP}} \\
Li~\cite{li2024halueval} & 22/30 & 8/30 & No strict/norm (12), no cost (24--26) \\
Min~\cite{min2023factscore} & 21/30 & 9/30 & No label vocab audit (17--18), no cost \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key findings across 6 papers}: (1)~\emph{Zero} papers pass Item~12 (strict vs.\ normalized F1 distinction)---confirming this is a universal blind spot. (2)~\emph{Zero} papers perform label vocabulary audits (Items~17--18), even when evaluating hallucination detection, because the concept of \emph{label} hallucination (as opposed to content hallucination) is absent from their frameworks. (3)~Cost transparency (Items~24--26) is failed by all 6 papers. (4)~Scores range from 11 to 22 out of 30, with higher-venue papers (EMNLP, arXiv with replication packages) scoring better on data integrity and statistics, but universally failing LLM-specific items.

\textbf{Cross-domain pattern}: The 5 LLM-specific items introduced by our checklist (Items 12, 17--20) are failed by 100\% of audited papers, regardless of domain or venue prestige. This validates our central claim that existing evaluation practices---including those at top venues---do not address label compliance failure modes.
...
\begin{table}[!ht]
\centering
\caption{Classification of checklist items into automated (scriptable) vs.\ manual (requires expert judgment) categories}
...
P20 (Cross-domain) & 26/30 & Partial multi-seed \\
P21 (Sub-1B models) & 26/30 & Single seed (SALAD) \\
...
\begin{enumerate}
\item \textbf{Automated tooling}: Python package automating Items 1--5, 10--13, 17--18, following the open-science principles advocated by Muennighoff et~al.~\cite{muennighoff2024}.
\item \textbf{Cross-domain validation}: Test checklist on clinical NLP (ICD-10) and legal classification.
\item \textbf{Journal integration}: Propose Items 29--30 as mandatory supplementary for LLM venues, complementing Raschka's~\cite{raschka2024} evaluation best practices.
...
The checklist itself, all case study data, and automated validation scripts are available at \url{https://github.com/nutthakorn7/soc-llm-research}.
...
\bibitem{li2024halueval} J.~Li et~al., ``HaluEval: A Large-Scale Hallucination Evaluation Benchmark for Large Language Models,'' in \emph{Proc.\ EMNLP}, 2023.
\bibitem{min2023factscore} S.~Min et~al., ``FActScore: Fine-grained Atomic Evaluation of Factual Precision in Long Form Text Generation,'' in \emph{Proc.\ EMNLP}, 2023.
\bibitem{wang2024} S.~Wang et~al., ``A Survey on Data Selection for Language Models,'' \emph{arXiv:2402.16827}, 2024.
\bibitem{tikhonov2024} A.~Tikhonov and I.~Yamshchikov, ``On the Reproducibility of Evaluation Results in NLP,'' \emph{Findings of EACL}, 2024.
\bibitem{biderman2024} S.~Biderman et~al., ``Lessons from the Trenches on Reproducible Evaluation of Language Models,'' \emph{arXiv:2405.14782}, 2024.
\bibitem{sun2024} T.~Sun et~al., ``A Survey of Safety and Trustworthiness of Large Language Models through the Lens of Verification and Validation,'' \emph{Artificial Intelligence Review}, vol.~57, no.~7, 2024.
\bibitem{raschka2024} S.~Raschka, ``LLM and AI Evaluation Best Practices,'' in \emph{Ahead of AI Magazine}, 2024.
\bibitem{secure2024} M.~Bhatt et~al., ``SECURE: Benchmarking Large Language Models for Cybersecurity,'' \emph{arXiv:2405.20441}, 2024.
\bibitem{liu2024dora} S.~Liu et~al., ``DoRA: Weight-Decomposed Low-Rank Adaptation,'' in \emph{Proc.\ ICML}, 2024.
\bibitem{zhang2023adalora} Q.~Zhang et~al., ``AdaLoRA: Adaptive Budget Allocation for Parameter-Efficient Fine-Tuning,'' in \emph{Proc.\ ICLR}, 2023.
\bibitem{muennighoff2024} N.~Muennighoff et~al., ``OLMo: Accelerating the Science of Language Models,'' in \emph{Proc.\ ACL}, 2024.
\end{thebibliography}
