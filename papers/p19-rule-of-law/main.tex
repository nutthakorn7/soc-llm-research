...
\begin{IEEEkeywords}
reproducibility, LLM evaluation, label aliasing, hallucination audit, cybersecurity NLP
\end{IEEEkeywords}
...
P14 (OFT vs.\ LoRA) & 25/30 & Cost breakdown \\
P15 (Multi-task) & 25/30 & Cost incomplete \\
P20 (Cross-domain) & 24/30 & Single seed, no CI \\
P21 (Sub-1B models) & 24/30 & Single seed, cost \\
P24 (Datasets) & 24/30 & Single seed, cost \\
P23 (Edge quant) & 23/30 & Single seed \\
P18 (Zero-shot) & 22/30 & Single seed, cost \\
P9 (DPO failure) & 21/30 & Single $\beta$, seed \\
\midrule
\textbf{Mean} & \textbf{25.0/30} & \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key findings}: (1)~The most common failure is \emph{limited seed diversity} (Items 14--16), a compute-constrained decision, not an oversight. (2)~Papers with multi-seed designs (P3, P6, P22, P15) score $\geq$25/30. (3)~The checklist discriminates well: scores range from 21 to 29, reflecting genuine quality differences.
...
\item \textbf{Comparison with existing frameworks}: 5 novel items not present in Pineau~\cite{pineau2021} or HELM~\cite{liang2022}.
\end{enumerate}
