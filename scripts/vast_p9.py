#!/usr/bin/env python3
"""P9 DPO β Sweep — Vast.ai standalone .py (no heredoc)"""
...
def train_sft(seed=42, epochs=3, lr=2e-4, batch=4, gacc=8, rank=64, ml=512):
    ...
    for d in salad_train:
        c = d["conversations"]
        msgs = [{"role":"system","content":c[0]["value"]},{"role":"user","content":c[1]["value"]},{"role":"assistant","content":c[2]["value"]}]
...
def eval_mdl(mdl, tok, n=500, ml=512):
    mdl.eval(); ps,gs,errs=[],[],[]
    sys_msg = salad_train[0]["conversations"][0]["value"]
    for i,d in enumerate(salad_test[:n]):
...
sft_f1,sft_acc,sft_errs = eval_mdl(sft_mdl, sft_tok)
